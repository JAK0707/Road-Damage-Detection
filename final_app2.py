...
if uploaded_file and concrete_cost_per_m3 and labour_cost and other_costs:
    if uploaded_file.name.endswith(('jpg', 'jpeg', 'png')):
    ...
    elif uploaded_file.name.endswith(('mp4','avi', 'mov')) and not st.session_state['processed']:
        ...
        if real_world_length_for_frame != 0:
            ...
            real_world_area = st.session_state['total_damage_volume_m3']/0.2
            pothole_volume = st.session_state['total_damage_volume_m3']
            total_material_cost = pothole_volume * concrete_cost_per_m3
            total_cost = total_material_cost + labour_cost + other_costs
            ...
            # Generate report button
            if st.button("Generate Report"):
                st.session_state['area_list'].append(0.9 * real_world_area)
                st.session_state['area_list'].append(1.1 * real_world_area)
                st.session_state['area_list'].append(real_world_area)
                st.session_state['cost_list'].append(0.9 * real_world_area * 0.2 * concrete_cost_per_m3)
                st.session_state['cost_list'].append(1.1 * real_world_area * 0.2 * concrete_cost_per_m3)
                st.session_state['cost_list'].append(total_material_cost)

                # Call report generation
                generate_report(st.session_state['area_list'], st.session_state['cost_list'], labour_cost, other_costs)
